...
LinkedListQueue* create_queue(int capacity) {
    ...
    queue->head->queue = (int*)malloc(capacity * sizeof(int));
    queue->head->capacity = capacity;
    queue->head->size = 0;
...
void enqueue(LinkedListQueue* queue, int value) {
    if (queue->tail->size < queue->capacity) {
        ...
        new_node->queue = (int*)malloc(queue->capacity * sizeof(int));
        new_node->capacity = queue->capacity;
        new_node->size = 0;
        new_node->queue[new_node->size++] = value;
