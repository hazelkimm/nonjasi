...
class LinkedListNode:
    """링크드 리스트 노드 클래스 (각 노드가 큐를 포함)"""
    def __init__(self):
        self.queue = Queue()  # 큐를 포함
        self.next = None      # 다음 링크드 리스트 노드를 가리키는 포인터


class DataStructure:
    """링크드 리스트로 연결된 큐의 데이터 구조"""
    def __init__(self):
    ...
    def push(self, value):
        ...
        if current.queue.size >= Queue.QUEUE_CAPACITY:
        ...
        print(f"Pushed value {value} to queue. Queue size is now {current.queue.size}.")
    ...
    def pop_and_redistribute(self):
        ...
        while self.head.queue.size < Queue.QUEUE_CAPACITY and self.head.next is not None:
            next_node = self.head.next
            if not next_node.queue.is_empty():
                redistributed_value = next_node.queue.dequeue()
                self.head.queue.enqueue(redistributed_value)
