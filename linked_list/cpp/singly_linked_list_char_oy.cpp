...
//    - 노드와 단일 연결 리스트 초기화 (노드 값으로 char 타입 사용)
// 2. insert_at_index
//    - 특정 인덱스에 노드를 삽입하는 함수
// 3. delete_at_index
//    - 특정 인덱스의 노드를 삭제하는 함수
// 4. find
//    - 리스트에서 특정 값(char)이 처음 나타나는 위치(인덱스)를 반환하는 함수
// 5. delete_value
...
// 8. contains
//    - 단일 연결 리스트에서 특정 값(char)이 있는지 확인하는 함수
// 9. nth_from_end
...
// Node 클래스 정의 (data 타입을 int에서 char로 변경)
class Node {
public:
    char data;  // int 대신 char 타입으로 변경
    Node* next;
...
public:
    ...
    unordered_map<char, int> count_values() {
        unordered_map<char, int> value_count;
        Node* current = head;
...
int main() {
    ...
    sll.insert_at_index(0, 'a');
    sll.insert_at_index(1, 'b');
    sll.insert_at_index(2, 'a');
    sll.insert_at_index(3, 'c');
    sll.insert_at_index(4, 'b');
    ...
    cout << "Checking if list contains value 'b': " << (sll.contains('b') ? "Yes" : "No") << endl;
    ...
    // 값의 등장 횟수를 세는 함수 호출
    unordered_map<char, int> value_count = sll.count_values();
